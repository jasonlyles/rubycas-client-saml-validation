module CASClient
  module XmlResponse
    ...
    def check_and_parse_xml(raw_xml)
      #This is currently the way I'm trying to determine if I have a valid CAS response or the SOAP response I get from the samlValidate uri
      if raw_xml.match(/^\<cas:serviceResponse/).to_s.empty?
        raw_xml = check_and_parse_soap(raw_xml)
      end
      begin
    ...
    def check_and_parse_soap(raw_xml)
      beginning_of_cas = "<cas:serviceResponse xmlns:cas='http://www.yale.edu/tp/cas'><cas:authenticationSuccess>"
      end_of_cas = "</cas:authenticationSuccess></cas:serviceResponse>"
      middle_of_cas = ""
      #Trickery and hacki-ness
      #Savon::SOAP::Response expects a HTTPI::Response object, so I had to fake one...
      response = HTTPI::Response.new(200, {}, raw_xml)
      soapy_response = Savon::SOAP::Response.new(response)
      #...in order to turn the output into a usable hash
      soap_hash = soapy_response.to_hash
      array_of_interest = soap_hash[:response][:assertion][:attribute_statement][:attribute]
      array_of_interest.each do |value|
        #Filter out LoginAccountResp, GetAccountResp and Password, which I don't need or want
